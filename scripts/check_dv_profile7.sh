...
convert_dv_profile() {
    ...
    # Step 4a: Convert TrueHD Atmos to DDP Atmos and preserve all other streams
    local ddp_audio="${temp_dir}/audio_ddp.eac3"
    
    echo -e "${BLUE}Converting TrueHD Atmos to DDP Atmos...${NC}"
    # Convert TrueHD track 1 to DDP Atmos (save as raw EAC-3 stream)
    # First try with 7.1 layout, fallback to copying original channels if that fails
    ffmpeg -i "$working_file" -map 0:a:0 -c:a eac3 -b:a 1536k -ac 8 "$ddp_audio" -y 2>/dev/null
    if [ $? -ne 0 ]; then
        echo -e "${YELLOW}7.1 conversion failed, trying with original channel layout...${NC}"
        # Fallback: convert without forcing channel layout
        ffmpeg -i "$working_file" -map 0:a:0 -c:a eac3 -b:a 1536k "$ddp_audio" -y 2>/dev/null
        if [ $? -ne 0 ]; then
            echo -e "${RED}Error: Failed to convert TrueHD to DDP${NC}"
            echo -e "${RED}Showing ffmpeg error output:${NC}"
            ffmpeg -i "$working_file" -map 0:a:0 -c:a eac3 -b:a 1536k "$ddp_audio" -y
            rm -rf "$temp_dir"
            return 1
        fi
    fi
    
    # Step 4b: Combine everything using mkvmerge with proper stream mapping
    echo -e "${BLUE}Creating final MKV with all tracks preserved...${NC}"
    
    # Use mkvmerge to combine:
    # 1. Converted video (HEVC with Profile 8 DV)
    # 2. Converted audio (DDP Atmos) - replaces first audio track
    # 3. All other audio tracks from original (starting from track 2)
    # 4. All subtitle tracks from original
    # 5. All chapters from original
    
    mkvmerge -o "$output_file" \
        --video-tracks 0 "$converted_hevc" \
        --audio-tracks 0 --language 0:eng --track-name "0:DDP Atmos 7.1" "$ddp_audio" \
        --no-video --audio-tracks 2- --subtitle-tracks 0- --chapters "$working_file" 2>/dev/null
    
    if [ $? -ne 0 ]; then
        echo -e "${YELLOW}Complex merge failed, trying alternative approach...${NC}"
        # Alternative approach: merge step by step
        local temp_video_audio="${temp_dir}/video_audio.mkv"
        
        # First combine video and converted audio
        mkvmerge -o "$temp_video_audio" "$converted_hevc" --language 0:eng --track-name "0:DDP Atmos" "$ddp_audio" 2>/dev/null
        if [ $? -ne 0 ]; then
            echo -e "${RED}Error: Failed to merge video and converted audio${NC}"
            rm -rf "$temp_dir"
            return 1
        fi
        
        # Then add all other streams from original
        mkvmerge -o "$output_file" "$temp_video_audio" --no-video --audio-tracks 2- --subtitle-tracks 0- --chapters "$working_file" 2>/dev/null
        if [ $? -ne 0 ]; then
            echo -e "${YELLOW}Failed to add all streams, creating file with video and converted audio only...${NC}"
            cp "$temp_video_audio" "$output_file"
            if [ $? -ne 0 ]; then
                echo -e "${RED}Error: Failed to create output file${NC}"
                rm -rf "$temp_dir"
                return 1
            fi
            echo -e "${YELLOW}Note: Only video and converted audio included (other streams could not be added)${NC}"
        else
            echo -e "${GREEN}Successfully preserved all original tracks!${NC}"
        fi
    else
        echo -e "${GREEN}Successfully preserved all original tracks!${NC}"
    fi
